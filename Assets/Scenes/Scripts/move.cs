<<<<<<< Updated upstream:Assets/Scenes/Scripts/move.cs
...
public class move : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, movePoint.position) <= .05f)
        {
            if (Mathf.Abs(hor) == 1f)
            {
                if (!Physics2D.OverlapCircle(movePoint.position + new Vector3(hor, 0f, 0f), 0.2f, whatStopMovement))
                {
            ...
            {
                if (!Physics2D.OverlapCircle(movePoint.position + new Vector3(ver, 0f, 0f), 0.2f, whatStopMovement))
                {
...
//   }
//}
>>>>>>> Stashed changes:Assets/move.cs
